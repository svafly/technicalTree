...
所以，第一个例子中的 T(n) = O(2n+2)，第二个例子中的 T(n) = O(2n2+2n+3)。这就是<b>大 O 时间复杂度表示法</b>。大 O 时间复杂度实际上并不具体表示代码真正的执行时间，而是表示<b>代码执行时间随数据规模增长的变化趋势</b>，所以，也叫作<b>渐进时间复杂度</b>（asymptotic time complexity），简称<b>时间复杂度</b>。
